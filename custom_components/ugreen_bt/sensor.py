...
from .ugreen_bt_lib.field_attributes import FIELD_ATTRIBUTES, PACK_FIELD_ATTRIBUTES, FieldType
from .ugreen_bt_lib.utils.device_builder import build_device
...
async def async_setup_entry(
    ...
    ugreen_device = build_device(address, device_name)

    sensors_to_add = []
    all_fields = FIELD_ATTRIBUTES

    if len(ugreen_device.pack_polling_commands) > 0:
        # add pack fields for device
        _LOGGER.info("Device type(%s) pack_num_max(%s)", ugreen_device.type, ugreen_device.pack_num_max)
        for pack in range (1, ugreen_device.pack_num_max + 1):
    ...
    for field_key, field_config in all_fields.items():
        if ugreen_device.has_field(field_key) or (len(ugreen_device.pack_polling_commands) > 0 and field_key.startswith("pack_")):
            category = None
...
class UgreenSensor(CoordinatorEntity, SensorEntity):
    """Ugreen universal sensor."""
