...
from ..base_devices.UgreenDevice import UgreenDevice
...
class DeviceReader:
    def __init__(
        ...
        self.ugreen_device = ugreen_device
    ...
    async def read_data(
        ...
        if self.ugreen_device is None:
            _LOGGER.error("Device is None")
            return None

        polling_commands = self.ugreen_device.polling_commands
        pack_commands = self.ugreen_device.pack_polling_commands
                ...
                async with async_timeout.timeout(self.polling_timeout):
                    ...
                    for command in polling_commands:
                        try:
                            ...
                            parsed = self.ugreen_device.parse(
                    ...
                    if len(pack_commands) > 0 and len(self.ugreen_device.pack_num_field) == 1:
                        _LOGGER.debug("Polling battery packs")
                        for pack in range(1, self.ugreen_device.pack_num_max + 1):
                            _LOGGER.debug("Setting pack_num to %i", pack)

                            # Set current pack number
                            command = self.ugreen_device.build_setter_command(
                                "pack_num", pack
                            )
                            body = command.parse_response(
                                await self._async_send_command(command)
                            ...
                            if self.ugreen_device.pack_num_max > 1:
