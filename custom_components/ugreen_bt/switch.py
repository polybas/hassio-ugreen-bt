...
from .ugreen_bt_lib.base_devices.UgreenDevice import UgreenDevice
from .ugreen_bt_lib.const import WRITE_UUID
...
async def async_setup_entry(
    ...
    for field_key, field_config in all_fields.items():
        if ugreen_device.has_field(field_key):
...
class UgreenSwitch(CoordinatorEntity, SwitchEntity):
    """Ugreen universal switch."""

    def __init__(
        self,
        ugreen_device: UgreenDevice,
        ...
        self._ugreen_device = ugreen_device
    ...
    async def write_to_device(self, state: bool):
        """Write to device."""
        command = self._ugreen_device.build_setter_command(self._response_key, state)
